...
from backend.services.query_service import get_candidates, get_capacity, get_import_history
...
app=FastAPI(title="Programador de Mantenimiento API",version="1.0.0")
...
@app.get("/api/health")
def health(): return {"ok":True,"service":"programador-mantenimiento"}
...
def candidates(specialty:str,year:int|None=None,month:int|None=Query(None,ge=1,le=12),area:str|None=None,criticality:str|None=None,condition:str|None=None,plan_search:str|None=None,limit:int=Query(500,ge=1,le=2000)):
    return get_candidates(specialty=specialty,year=year,month=month,area=area,criticality=criticality,condition=condition,plan_search=plan_search,limit=limit)
...
def create_programming(body:ProgrammingCreate):
    try: return save_programming(date_from=body.date_from,date_to=body.date_to,specialty=body.specialty,pmp_ids=body.pmp_ids,created_by=body.created_by,reason=body.reason)
...
def close_program(body:ProgrammingClose):
    ...
    try: return close_programming(programming_id=body.programming_id,version_id=body.version_id,reasons=reason_map,closed_by=body.closed_by)
    except ProgrammingError as exc: raise HTTPException(422,str(exc)) from exc
