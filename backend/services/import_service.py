...
def _begin_import(conn, tipo, filename, content, **meta):
    return conn.execute(text("""INSERT INTO importacion
...
def _finish(conn, iid, read, inserted, updated, rejected, warnings=None):
    ...
    conn.execute(text("""UPDATE importacion SET estado=:e,filas_leidas=:r,filas_insertadas=:i,
...
def _error(conn,iid,row,entity,key,column,value,message,severity="ERROR"):
    conn.execute(text("""INSERT INTO importacion_error
...
def _spec_id(conn, code):
    return conn.execute(text("SELECT id FROM especialidad WHERE codigo=:c"),{"c":code}).scalar_one_or_none()

def _plan_id(conn,spec,canonical):
    return conn.execute(text("""SELECT p.id FROM plan_trabajo p
      JOIN especialidad e ON e.id=p.especialidad_id
...
def import_master_assets(filename,content):
    ...
    with get_engine().begin() as conn:
        ...
        for r in parsed.rows:
            ...
            exists=conn.execute(text("SELECT id FROM activo WHERE codigo=:c"),{"c":r["code"]}).scalar_one_or_none()
            conn.execute(text("""INSERT INTO activo
              ...
              especialidad_id=COALESCE(EXCLUDED.especialidad_id,activo.especialidad_id),
        ...
        for child,parent in parent_links:
            conn.execute(text("""UPDATE activo a SET activo_padre_id=p.id FROM activo p
...
def import_master_plans(filename,content):
    ...
    with get_engine().begin() as conn:
        ...
        for r in parsed.rows:
            ...
            gid=conn.execute(text("""INSERT INTO grupo_plan_trabajo(especialidad_id,nombre)
            ...
            pid=conn.execute(text("""INSERT INTO plan_trabajo(especialidad_id,grupo_id,nombre,nombre_canonico,numero_personas)
              ...
              numero_personas=COALESCE(EXCLUDED.numero_personas,plan_trabajo.numero_personas),
...
def import_master_classification(filename,content):
    ...
    with get_engine().begin() as conn:
        ...
        for r in parsed.rows:
            ...
            exists=conn.execute(text("SELECT id FROM plan_trabajo WHERE id=:p"),{"p":pid}).scalar_one_or_none()
...
def import_master_personnel_turns(filename,content):
    ...
    with get_engine().begin() as conn:
        ...
        for r in persons.rows:
            ...
            exists=conn.execute(text("SELECT id FROM tecnico WHERE nombre_normalizado=:n"),{"n":r["normalized"]}).scalar_one_or_none()
            tid=conn.execute(text("""INSERT INTO tecnico(nombre,nombre_normalizado,especialidad_id,activo)
            ...
            conn.execute(text("""INSERT INTO tecnico_alias(tecnico_id,alias_normalizado) VALUES(:t,:a)
              ON CONFLICT(alias_normalizado) DO UPDATE SET tecnico_id=EXCLUDED.tecnico_id"""),{"t":tid,"a":r["normalized"]})
            upd += 1 if exists else 0; ins += 0 if exists else 1
        for r in turns.rows:
            exists=conn.execute(text("SELECT id FROM turno WHERE codigo=:c"),{"c":r["code"]}).scalar_one_or_none()
            tid=conn.execute(text("""INSERT INTO turno(codigo,codigo_estandar,horas_disponibles,es_ausencia,activo)
            ...
            conn.execute(text("""INSERT INTO turno_alias(alias_codigo,turno_id) VALUES(:a,:t)
...
def import_monthly_planning(filename,content):
    ...
    with get_engine().begin() as conn:
        ...
        period=conn.execute(text("""INSERT INTO periodo_mensual(anio,mes) VALUES(:y,:m)
          ON CONFLICT(anio,mes) DO UPDATE SET anio=EXCLUDED.anio RETURNING id"""),{"y":year,"m":month}).scalar_one()
        for r in parsed.rows:
            sid=_spec_id(conn,r["specialty"]); aid=conn.execute(text("SELECT id FROM activo WHERE codigo=:c"),{"c":r["asset_code"]}).scalar_one_or_none()
            pid=_plan_id(conn,r["specialty"],r["plan_canonical"])
            missing=[]
            ...
            exists=conn.execute(text("SELECT id FROM pmp WHERE periodo_mensual_id=:pm AND source_key=:k"),{"pm":period,"k":r["source_key"]}).scalar_one_or_none()
            pmpid=conn.execute(text("""INSERT INTO pmp(periodo_mensual_id,source_key,activo_id,plan_trabajo_id,especialidad_id,
            ...
            if r["order_number"]:
                conn.execute(text("""INSERT INTO orden_mantenimiento(numero_orden,pmp_id,estado,fecha_finalizacion,importacion_id_ultima)
                  VALUES(:n,:p,:e,:f,:i) ON CONFLICT(numero_orden) DO UPDATE SET pmp_id=EXCLUDED.pmp_id,
                  estado=CASE WHEN orden_estado='FINALIZADA' THEN 'FINALIZADA' ELSE EXCLUDED.estado END,
                  fecha_finalizacion=COALESCE(EXCLUDED.fecha_finalizacion,orden_fecha_finalizacion),
...
def import_technician_roster(filename,content,year=None,month=None):
    ...
    with get_engine().begin() as conn:
        ...
        for r in parsed.rows:
            tech=conn.execute(text("""SELECT id FROM tecnico WHERE nombre_normalizado=:n
              UNION SELECT tecnico_id FROM tecnico_alias WHERE alias_normalizado=:n LIMIT 1"""),{"n":r["technician_normalized"]}).scalar_one_or_none()
            turn=conn.execute(text("""SELECT id FROM turno WHERE codigo=:c
              UNION SELECT turno_id FROM turno_alias WHERE alias_codigo=:c LIMIT 1"""),{"c":r["turn_code"]}).scalar_one_or_none()
            ...
            exists=conn.execute(text("SELECT id FROM programacion_tecnico WHERE tecnico_id=:t AND fecha=:f"),{"t":tech,"f":r["date"]}).scalar_one_or_none()
            conn.execute(text("""INSERT INTO programacion_tecnico(tecnico_id,fecha,turno_id,importacion_id)
...
def import_order_states(filename,content):
    ...
    with get_engine().begin() as conn:
        ...
        for r in parsed.rows:
            oid=conn.execute(text("SELECT id FROM orden_mantenimiento WHERE numero_orden=:n"),{"n":r["order_number"]}).scalar_one_or_none()
            if not oid:
                oid=conn.execute(text("""INSERT INTO orden_mantenimiento(numero_orden,estado,fecha_finalizacion,importacion_id_ultima)
                  VALUES(:n,:e,:f,:i) RETURNING id"""),{"n":r["order_number"],"e":r["state"],"f":r["finished_at"],"i":iid}).scalar_one(); ins+=1
            else:
                conn.execute(text("""UPDATE orden_mantenimiento SET estado=:e,fecha_finalizacion=COALESCE(:f,fecha_finalizacion),
                  importacion_id_ultima=:i,actualizado_en=NOW() WHERE id=:id"""),{"e":r["state"],"f":r["finished_at"],"i":iid,"id":oid}); upd+=1
            conn.execute(text("""INSERT INTO orden_estado_historial(orden_id,estado,fecha_finalizacion,importacion_id)
