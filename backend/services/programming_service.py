...
def analyze_programming_closure(*,version_id:int,content:bytes)->dict:
    ...
    master_orders=parsed["orders"].rows
    if not master_orders:
        raise ProgrammingError("El archivo no contiene órdenes mensuales válidas")

    state_by_order={}
    for row in master_orders:
        order=(row.get("order_number") or "").strip().upper()
        if order:
            state_by_order[order]=(row.get("state") or "PENDIENTE").strip().upper()
    ...
    with get_engine().connect() as conn:
        ...
        rows=conn.execute(text("""SELECT
          ...
          om.numero_orden,a.codigo activo_codigo,a.descripcion activo_descripcion,
    ...
    for row in rows:
        ...
        hh_programmed+=hh
        order=(item.get("numero_orden") or "").strip().upper()
        master_state=state_by_order.get(order) if order else None

        if master_state=="FINALIZADA":
            result="FINALIZADA"
            finalized+=1
            hh_finalized+=hh
        elif master_state=="PENDIENTE":
            result="PENDIENTE"
            pending+=1
        else:
            result="REVISAR"
        ...
        item["estado_maestro"]=master_state or "NO ENCONTRADA"
        item["resultado_cierre"]=result
        detail.append(item)

    total=len(detail)
    compliance=(finalized/total*100) if total else 0.0
...
def analyze_programming_closure_from_db(*,version_id:int)->dict:
    with get_engine().connect() as conn:
        header=conn.execute(text("""SELECT
          ps.id programming_id,pv.id version_id,pv.numero_version,
          per.fecha_desde,per.fecha_hasta,e.codigo especialidad,ps.estado
        ...
        rows=conn.execute(text("""SELECT
          pi.id program_item_id,pi.hh_programadas,
          om.numero_orden,COALESCE(om.estado,'NO ENCONTRADA') estado_maestro,
    ...
    for row in rows:
        ...
        hh=float(item.get("hh_programadas") or 0)
        hh_programmed+=hh
        master_state=(item.get("estado_maestro") or "NO ENCONTRADA").strip().upper()
        item["estado_maestro"]=master_state

        if master_state=="FINALIZADA":
            item["resultado_cierre"]="FINALIZADA"
            finalized+=1
            hh_finalized+=hh
        elif master_state=="PENDIENTE":
            item["resultado_cierre"]="PENDIENTE"
            pending+=1
        else:
            item["resultado_cierre"]="REVISAR"
            review+=1
        detail.append(item)
