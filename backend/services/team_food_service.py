...
def _begin_import(conn,filename:str,year:int,month:int,total:int):
    iid=conn.execute(text("""INSERT INTO importacion(tipo,nombre_archivo,estado,anio,mes,filas_leidas)
      VALUES('MAESTRO_TEAM_FOOD',:f,'PROCESANDO',:y,:m,:n) RETURNING id"""),{"f":filename,"y":year,"m":month,"n":total}).scalar_one()
    sid=conn.execute(text("""INSERT INTO sincronizacion_fuente_maestra(fuente,referencia,anio,mes,estado,filas_leidas)
...
def import_team_food(filename:str,content:bytes,*,year:int|None=None,month:int|None=None)->dict[str,Any]:
    ...
    with get_engine().begin() as conn:
        ...
        spec_ids={r["codigo"]:int(r["id"]) for r in conn.execute(text("SELECT id,codigo FROM especialidad")).mappings()}
        ...
        if technician_rows:
            conn.execute(text("UPDATE tecnico SET activo=false,codigo_externo=NULL"))
            _execute_many(conn,"""INSERT INTO tecnico(codigo_externo,nombre,nombre_normalizado,especialidad_id,activo)
              VALUES(:external,:name,:normalized,:sid,true)
              ON CONFLICT(nombre_normalizado) DO UPDATE SET
              codigo_externo=COALESCE(excluded.codigo_externo,tecnico.codigo_externo),
        ...
        _execute_many(conn,"""INSERT INTO activo(codigo,descripcion,criticidad,especialidad_id,estado_activo,area_nombre,linea_nombre,activo,actualizado_en)
          ...
          criticidad=COALESCE(excluded.criticidad,activo.criticidad),
          especialidad_id=COALESCE(excluded.especialidad_id,activo.especialidad_id),
          estado_activo=COALESCE(excluded.estado_activo,activo.estado_activo),
          area_nombre=COALESCE(excluded.area_nombre,activo.area_nombre),
          linea_nombre=COALESCE(excluded.linea_nombre,activo.linea_nombre),actualizado_en=now()""",asset_rows)
        processed+=len(asset_rows)
        asset_map={str(r["codigo"]).strip().upper():dict(r) for r in conn.execute(text("SELECT id,codigo FROM activo")).mappings()}
        ...
        _execute_many(conn,"""INSERT INTO grupo_plan_trabajo(especialidad_id,nombre,activo)
          VALUES(:sid,:name,true) ON CONFLICT(especialidad_id,nombre) DO UPDATE SET activo=true""",list(groups.values()))
        group_map={(r["especialidad_id"],r["nombre"]):int(r["id"]) for r in conn.execute(text("SELECT id,especialidad_id,nombre FROM grupo_plan_trabajo")).mappings()}
        ...
        _execute_many(conn,"""INSERT INTO plan_trabajo(especialidad_id,grupo_id,nombre,nombre_canonico,numero_personas,equipo_detenido,tiempo_ejecucion_min,tiempo_parada_min,fuente_maestra,activo)
          ...
          equipo_detenido=COALESCE(excluded.equipo_detenido,plan_trabajo.equipo_detenido),
          tiempo_ejecucion_min=COALESCE(excluded.tiempo_ejecucion_min,plan_trabajo.tiempo_ejecucion_min),
        ...
        plan_records=list(conn.execute(text("""SELECT p.id,p.nombre_canonico,p.tiempo_ejecucion_min,p.numero_personas,e.codigo especialidad
          FROM plan_trabajo p JOIN especialidad e ON e.id=p.especialidad_id""")).mappings())
        ...
        _execute_many(conn,"""INSERT INTO actividad_maestra(activo_id,plan_trabajo_id,especialidad_id,fuente_datos,importacion_id_ultima,activo,actualizado_en)
        ...
        period_id=conn.execute(text("""INSERT INTO periodo_mensual(anio,mes) VALUES(:y,:m)
        ...
        _execute_many(conn,"""UPDATE activo SET area_nombre=COALESCE(:area,area_nombre),
          criticidad=COALESCE(:crit,criticidad),actualizado_en=now() WHERE codigo=:code""",asset_updates)
        _execute_many(conn,"""INSERT INTO pmp(periodo_mensual_id,source_key,activo_id,plan_trabajo_id,especialidad_id,titulo,fecha_planeada_inicio,cronograma_planeacion,tiempo_planeado_min,importacion_id_ultima)
        ...
        pmp_map={r["source_key"]:int(r["id"]) for r in conn.execute(text("SELECT id,source_key FROM pmp WHERE periodo_mensual_id=:p"),{"p":period_id}).mappings()}
        order_rows=[]
        ...
        _execute_many(conn,"""INSERT INTO orden_mantenimiento(numero_orden,pmp_id,estado,importacion_id_ultima,actualizado_en)
        ...
        _execute_many(conn,"""INSERT INTO turno(codigo,codigo_estandar,horas_disponibles,es_ausencia,activo)
        ...
        turn_map={r["codigo"]:int(r["id"]) for r in conn.execute(text("SELECT id,codigo FROM turno")).mappings()}
        ...
        tech_rows=list(conn.execute(text("SELECT id,codigo_externo,nombre_normalizado FROM tecnico WHERE activo=true")).mappings())
        ...
        _execute_many(conn,"""INSERT INTO programacion_tecnico(tecnico_id,fecha,turno_id,importacion_id,actualizado_en)
...
def learn_plan(*,plan_id:int,condition:str|None,people:float|None,updated_by:str|None=None)->dict[str,Any]:
    ...
    with get_engine().begin() as conn:
        if not conn.execute(text("SELECT id FROM plan_trabajo WHERE id=:p"),{"p":plan_id}).scalar_one_or_none():
